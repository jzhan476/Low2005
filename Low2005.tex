...
under flexible versus fixed labour supply. Retirement (ages 65--84) is appended as a
deterministic, closed-form continuation at the calibrated $0.55$ replacement rate
(see Section~\ref{sec:replication_figures} below and the discussion in the notebook for
the limitations of this approximation).

\section{Replication targets}\label{sec:replication-targets}

Table~\ref{tab:replication-targets} compares four quantitative working-life targets
that Low~(2005) reports for the baseline calibration with the values produced by
this replication. The numbers in the right-hand column are auto-generated by
\path{Code/Python/Low2005.py} and written to
\path{Figures/replication_summary.tex}, which the paper then \verb|\input|s, so
...
\begin{table}[htbp]
  ...
  \input{Figures/replication_summary.tex}
\end{table}

The replication matches the qualitative pattern of the paper on every comparison
the script examines: uncertainty raises hours early in life and raises peak
asset accumulation, and labour-supply flexibility reduces precautionary
saving relative to the fixed-hours benchmark
(see Section~\ref{sec:replication_figures}).

Quantitatively, mean hours and the peak-assets age are within a few
percent / a few years of the paper's targets, but both wealth-to-income ratios
come in at roughly 40\% of the paper's reported levels. Plausible sources of
this gap include (i) the closed-form deterministic retirement extension used
here in place of the paper's fully life-cycle-consistent retirement stage,
which mechanically shrinks middle-age precautionary balances; (ii) HARK's
\texttt{LaborIntMargConsumerType} normalization conventions for the wage
process, which differ in detail from the original Gauss implementation; and
(iii) a smaller borrowing-constraint specification than the paper uses. A more
faithful match of the level moments would require a HARK agent that supports
both a labour-flexibility margin and a zero-wage retirement stage in the same
backward-induction problem; this is left as a future extension and noted in
the notebook accompanying the script.
...
Retirement profiles (ages 65--84) are computed as a closed-form
certainty-equivalent extension: each simulated agent enters retirement
with their terminal assets and a constant pension, and consumes
optimally subject to the Euler equation, a no-borrowing constraint,
and no bequests. Because retirement is solved in isolation from the
working-life stochastic problem, there is a discrete upward movement
in mean consumption at age 65 -- agents ``discover'' the
certainty-equivalent of their accumulated precautionary buffer when
uncertainty resolves. A fully life-cycle-consistent solution with
labour flexibility and a zero-wage retirement stage is noted in the
notebook as a future extension. Vertical dotted lines in the figures
